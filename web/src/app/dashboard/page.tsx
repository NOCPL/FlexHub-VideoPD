...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err) => setError(err instanceof Error ? err.message : "Could not load visits."));
    return () => {
      cancelled = true;
    };
  }, [user]);
  ...
  async function schedule(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not schedule the visit.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="min-h-full bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-4">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Schedule a visit</h1>
            <p className="text-muted-foreground text-sm">
              This only generates links. Angular always opens the static host URL. Kotlin opens the field
              URL with bank, groupId, and memberId.
            </p>
          </div>
          {hostUrl ? (
            <Card>
              <CardHeader>
                <CardTitle>Static host URL</CardTitle>
                <CardDescription>
                  {user.role === "Admin" && selectedOfficer
                    ? `${selectedOfficer.name}'s permanent host URL. It does not change when you schedule.`
                    : "Does not change when you schedule another visit."}
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col gap-2 sm:flex-row">
                <code className="bg-muted flex-1 truncate rounded-md px-3 py-2 text-xs">{hostUrl}</code>
                <Button variant="outline" size="sm" onClick={() => copy(hostUrl, "Host URL")}>
                  Copy
                </Button>
                <Button size="sm" onClick={() => router.push(new URL(hostUrl).pathname)}>
                  Open room
                </Button>
              </CardContent>
            </Card>
          ...
          {meetings === null ? (
            ...
            <Card>
              <CardHeader>
                <CardTitle>No visits yet</CardTitle>
                <CardDescription>Use the form to mint a field-officer link.</CardDescription>
              </CardHeader>
            </Card>
          ) : (
            <>
              <VisitList title="Upcoming" items={upcoming} empty="No open visits." onCopy={copy} />
              <VisitList title="Completed" items={past} empty="No completed visits yet." onCopy={copy} />
            </>
          )}
        </section>
        ...
        <Card className="h-fit">
          <CardHeader>
            <CardTitle>Generate field-officer link</CardTitle>
            <CardDescription>Query parameters are stored as-is. Flexhub Video PD does not validate them.</CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-3" onSubmit={schedule}>
              ...
              <Button type="submit" className="w-full" disabled={busy || (user.role === "Admin" && !form.creditOfficerId)}>
            ...
            <CardContent className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-medium">{m.memberName || m.memberId || "Visit"}</span>
                  <Badge variant="secondary">{STATUS_LABEL[m.status] ?? m.status}</Badge>
                </div>
                <p className="text-muted-foreground mt-1 text-sm">
                  {m.bank} · {m.groupId} · {m.memberId} · {new Date(m.scheduledAt).toLocaleString()}
                </p>
